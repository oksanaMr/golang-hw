...
func TestInMemoryStorage_Create(t *testing.T) {
	...
	assert.ErrorIs(t, err, storage.ErrEventAlreadyExists)
}
...
func TestInMemoryStorage_ListByDay(t *testing.T) {
	...
	tomorrow := storage.Event{
		ID:        uuid.New(),
		Title:     "Tomorrow Event",
		EventTime: now.AddDate(0, 0, 1),
	}
	yesterday := storage.Event{
		ID:        uuid.New(),
...
func TestInMemoryStorage_ListByMonth(t *testing.T) {
	...
	thisMonth := storage.Event{
		ID:        uuid.New(),
		Title:     "This Month",
		EventTime: now,
	}
	thisMonth2 := storage.Event{
		ID:        uuid.New(),
		Title:     "This Month 2",
		EventTime: now.AddDate(0, 0, 15),
	}
	nextMonth := storage.Event{
		ID:        uuid.New(),
		Title:     "Next Month",
		EventTime: now.AddDate(0, 1, 0),
	}
	lastMonth := storage.Event{
		ID:        uuid.New(),
		Title:     "Last Month",
		EventTime: now.AddDate(0, -1, 0),
	}
